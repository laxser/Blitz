...
import com.laxser.blitz.web.ParamValidator;
import com.laxser.blitz.web.RequestPath;
import com.laxser.blitz.web.annotation.HttpFeatures;
import com.laxser.blitz.web.annotation.IfParamExists;
import com.laxser.blitz.web.annotation.Intercepted;
import com.laxser.blitz.web.annotation.Return;
import com.laxser.blitz.web.impl.module.Module;
import com.laxser.blitz.web.impl.validation.ParameterBindingResult;
import com.laxser.blitz.web.paramresolver.MethodParameterResolver;
import com.laxser.blitz.web.paramresolver.ParamMetaData;
import com.laxser.blitz.web.paramresolver.ParamResolver;
import com.laxser.blitz.web.paramresolver.ParameterNameDiscovererImpl;
import com.laxser.blitz.web.paramresolver.ResolverFactoryImpl;

/**
 *@author laxser  Date 2012-3-23 下午4:42:43
...
public final class ActionEngine implements Engine {
    ...
    private InterceptorDelegate[] compileInterceptors() {
        ...
        for (InterceptorDelegate interceptor : interceptors) {
            ...
            if (!most.getClass().getName().startsWith("net.paoding.blitz.web")) {
                ...
                if (intercepted != null) {
                    // 3.1 先排除deny禁止的
                    if (BlitzStringUtil.matches(intercepted.deny(), interceptor.getName())) {
                    ...
                    // 3.2 确认最大的allow允许
                    if (!BlitzStringUtil.matches(intercepted.allow(), interceptor.getName())) {
                        if (logger.isDebugEnabled()) {
                            logger.debug("action '" + controllerClass.getName() + "#"
                                    + method.getName()
    ...
    public Object execute(Blitz blitz) throws Throwable {
        try {
            return innerExecute(blitz);
        } catch (Throwable local) {
            throw createException(blitz, local);
        }
    }

    protected Object innerExecute(Blitz blitz) throws Throwable {
        ...
        return new InvocationChainImpl(blitz).doNext();
    }
    ...
    private class InvocationChainImpl implements InvocationChain {
        ...
        public InvocationChainImpl(Blitz blitz) {
            this.blitz = blitz;
        }
        ...
        public Object doNext() throws Exception {
            if (++index < interceptors.length) { // ++index 用于将-1转化为0
                ...
                //
                blitz.addAfterCompletion(interceptor);
                Object instruction = interceptor.roundInvocation(blitz.getInvocation(), this);
                //
                if (debugEnabled) {
                ...
                if (httpFeatures != null) {
                    applyHttpFeatures(blitz.getInvocation());
                }

                this.instruction = method.invoke(controller, blitz.getInvocation()
                        .getMethodParameters());
    ...
    private Exception createException(Blitz blitz, Throwable exception) {
        final RequestPath requestPath = blitz.getInvocation().getRequestPath();
        StringBuilder sb = new StringBuilder(1024);
        sb.append("[Blitz-").append(BlitzVersion.getVersion()).append("@Spring-").append(
        ...
        sb.append(Arrays.toString(blitz.getInvocation().getMethodParameters()));
    ...
    private String showSimpleName(Class<?> parameterType, String appPackageName) {
        if (parameterType.getName().startsWith("com.laxser")
