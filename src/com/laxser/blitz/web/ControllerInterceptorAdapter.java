		...
		DispatcherSelector
{
	...
	@Override
	public final boolean isForAction(Class<?> controllerClazz,
			Method actionMethod)
	{
	...
	@Override
	public final Object roundInvocation(Invocation inv, InvocationChain chain)
			throws Exception
	{
		// before
	 ...
	 * 返回其他的表示不再调用剩余的拦截器以及action，并按返回的指示执行结果(进行页面渲染或其他 ) {@link Instruction}
	 * 向请求发送响应(可以是用字符串表示的指示对象)。
	 * <p>
	 * 在一个拦截器链条中，如果某一个拦截器拒绝了整个调用链条，其它还没拦截的拦截器将不再会进行拦截，但是之前已经拦截过的拦截器， 还将分别调用它们的
	 * {@link #after(Invocation, Object)}和
	 ...
	 * @return
	 */
	protected final boolean checkRequiredAnnotations(Class<?> controllerClazz,
			Method actionMethod)
	{
		...
		for (Class<? extends Annotation> requiredAnnotation : requiredAnnotations) {
			...
			if (scopeSet.get(AnnotationScope.METHOD.ordinal())) {
				if (actionMethod.isAnnotationPresent(requiredAnnotation)) {
					return checkAnnotation(actionMethod
							.getAnnotation(requiredAnnotation));
				}
			}
	 ...
	 * @return
	 */
	protected BitSet getAnnotationScope(
	 ...
	 * @return
	 */
	protected final boolean checkDenyAnnotations(Class<?> controllerClazz,
			Method actionMethod)
	{
