 ...
 */
package com.laxser.blitz.scanning;
...
/**Blitz的扫描器，用来扫描是否有Blitz jar模块
 * @author laxser  Date 2012-3-23 下午3:14:28
@contact [duqifan@gmail.com]
@BlitzScanner.java

 */
public class BlitzScanner {

    private static SoftReference<BlitzScanner> softReference;

    public synchronized static BlitzScanner getInstance() {
        if (softReference == null || softReference.get() == null) {
            BlitzScanner blitzScanner = new BlitzScanner();
            softReference = new SoftReference<BlitzScanner>(blitzScanner);
        }
    ...
    public List<ResourceRef> getJarOrClassesFolderResources(String[] scope) throws IOException {
        if (logger.isInfoEnabled()) {
            logger.info("[findFiles] start to found classes folders "
                    + "or blitzd jar files by scope:" + Arrays.toString(scope));
        }
                    ...
                    if (!resources.contains(ref)) {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug("[findFiles] found classes folders "
                                    + "or blitzd jar files by scope:" + ref);
                        }
                    }
                }
            }
